...
import { buildFrameContinuum, getRecentFables, getRecentFrames } from "../memory";
...
async function submitInput(req: Request, brain: Brain): Promise<Response> {
  ...
  // Build conclusion for latest resulted frame
  buildFrameContinuum(brain, info);

  // Create frame
  const { data: newFrame } = await supabase.from("frame").insert({
    brain_id: brain.id,
    input: info,
    goal: goal,
  }).select().single() as { data: Frame | null };;
