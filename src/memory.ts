...
async function searchEntireMemory(fable: Fable): Promise<FableConnection[]> {
    ...
    for (const fable_connection of data) {
        ...
        const { error } = await supabase.from('recently_recalled').upsert(
            {
                ...
                remove_at: convertDateToSupabaseFormat(
                    new Date(new Date().getTime() + RECENT_MEMORY_TIME)
                ),
            },
            {
                onConflict: 'id_pair',
...
export async function searchMemory(fable: Fable) {
    ...
    if (usefulConnections.length < TOP_CONNECTIONS) {
        ...
        console.log(
            `Total of ${connectionMap.size} useful connections, up to ${TOP_CONNECTIONS}.`
        )
...
export async function getRecentFrames(brain: Brain, minutes = 10) {
    // In order of most recently created
    const { data } = await supabase
        .from('frame')
        .select('*')
...
export async function buildFrameContinuum(brain: Brain, newInput: string) {
    const recentFrames = await getRecentFrames(brain)

    if (recentFrames.length > 0) {
        const latestResultedFrame = recentFrames.find(
            (frame) => frame.resulting_action_id !== null
        )
        if (latestResultedFrame && !latestResultedFrame.conclusion) {
            const { data: latestResultedAction } = (await supabase
                .from('action')
                .select('*')
                .eq('id', latestResultedFrame.resulting_action_id)
            ...
            if (latestResultedAction) {
                let conclusion = await generateResponseGPT4(
                    ...
                    ## Previous Input: ${latestResultedFrame.input.trim()}
